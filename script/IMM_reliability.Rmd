...
### D-score difference correlation between measures for challenge and criticism
```{r}
# t test
...
```{r echo=TRUE}
results_rt <- final %>% 
  select(c(4:7, 12:13, 22:33)) %>% 
  corrr::correlate(method = "spearman") %>% 
  corrr::focus(matches("1$")) 
```

### Correlation between implicit and explicit measures (d-scores)
in the first round 
```{r echo=TRUE}
results_d_first <- final %>% 
  select(c(4:7, 12:21, 34:37)) %>% 
  corrr::correlate(method = "spearman") %>% 
  corrr::focus(matches("1$"), mirror = T)
```

and in the second round
```{r echo=TRUE}
results_d_second <- final %>% 
  select(c(4:7, 12:21, 34:37)) %>%
  corrr::correlate(method = "spearman") %>% 
  corrr::focus(matches("2$"), mirror = T)
```

### Correlation between implicit and explicit measures (rt)
in the first round 
```{r echo=TRUE}
results_rt_first <- final %>% 
  select(c(4:7, 12:13, 22:33)) %>% 
  corrr::correlate(method = "spearman") %>% 
  corrr::focus(matches("1$"), mirror = T)
```
...
# correlation by word category
words %>% 
  split(.$word_category) %>% 
  map(select, -c(word_category, word)) %>% 
  map(cor)
```
