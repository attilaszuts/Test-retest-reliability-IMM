...
rt_mean1 <- as.integer(gnat_filter1 %>% 
                         summarise(pers_a = mean(rt)))
rt_sd1 <- as.integer(gnat_filter1 %>% 
                       summarise(pers_sd = sd(rt)))
rt_filter1 <- (rt_sd1 * 3) + rt_mean1

rt_mean2 <- as.integer(gnat_filter2 %>% 
                         summarise(pers_a = mean(rt)))
rt_sd2 <- as.integer(gnat_filter2 %>% 
                       summarise(pers_sd = sd(rt)))
...
implicit <- 
  ...
  filter(measurement == 1,
         rt > 300 & rt < rt_filter1) %>% 
  filter(measurement == 2,
         rt > 300 & rt < rt_filter2) %>%
  ungroup() %>% 
  #calculating d-scores for round 1
  group_by(id) %>% 
  filter(measurement == 1)
