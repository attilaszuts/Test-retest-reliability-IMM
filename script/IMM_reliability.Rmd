...
final_day_diff <- final %>% 
  ...
  separate(name, c("block_1", "block_2", "measurement_type", "measurement_round")) %>% 
  unite(col = "block", c("block_1", "block_2"), sep = "_") %>% 
  pivot_wider(id_cols = c(id, measurement_round, days), names_from = c(block, measurement_type), values_from = value)
```
...
final_long %>% 
  filter(measurement_type == "d") %>% 
  ggplot(aes(block, value, fill = measurement_round)) + geom_boxplot(position = "dodge")
