# d-score by word category + sd(rt) based on blocks
implicit <- 
  gnat_raw %>% 
  ...
  ungroup() %>% 
  # filter overall and block error rates, filter too quick and too slow responses, filter non-target words
  filter(correct_block > 0.6 & correct_all > 0.8 & error == 0,
         rt > 300 & rt < 1399,
         word_category != "positive" & word_category != "negative") %>%
